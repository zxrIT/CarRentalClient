	...
	methods: {
		...
		onDayClick(e: any) {
			...
			if (this.data.isSelectingReturn) {
				...
				const app = getApp();
				app.globalData.duration = Math.max(1, duration)
				const timeOne = new Date(pickupTimestamp)
				const timeTwo = new Date(returnTimestamp)
				app.globalData.pickTime = timeOne
				app.globalData.returnTime = timeTwo
				this.setData({
					days,
